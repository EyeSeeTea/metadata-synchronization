...
-   **When a returning version looks alarming, check the advisory's range before keeping the pin.** An older version coming back is not by itself a reason to keep a constraint. In a sibling repository `glob-parent@3.1.0` returned when its entry was removed, and `GHSA-ww39-953v-wcq6` affects `>= 4.0.0, < 5.1.2` — the version that returned was never in range and the entry was protecting nothing.
